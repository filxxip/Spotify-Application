...
class LoginWindow:
    ...
    def entry_setter(self):
        with open(
            f"{Path(__main__.__file__).parent.__str__()}/json_files/data.json"
        ) as file:
            data = json.load(file)
    ...
    def check_json(self):
        with open(
            rf"{Path(__main__.__file__).parent.__str__()}/json_files/data.json"
        ) as file:
            data = json.load(file)
        ...
        if self.checkbox.state:
            ...
            with open(
                rf"{Path(__main__.__file__).parent.__str__()}/json_files/data.json", "w"
            ) as file:
                json.dump(data, file)
            return True
        return False
