...
from PyQt5.QtWidgets import (
    ...
    QHBoxLayout,
    QPushButton,
    QSizePolicy,QFormLayout, QScrollArea, QGroupBox
)
import json
...
class Play_button:
    def __init__(self, image=True):
        ...
        if image:
            self.image1 = QIcon("images/play.png")
            self.image2 = QIcon("images/stop.png")
            self.set_icon()
        else:
            self.image1 = QIcon("images/empty.png")
...
class ForLabel:
    def __init__(self, master, index, title, author, length, uncheck_function=None):
        ...
        if uncheck_function:
            ...
            self.playbutton.button.clicked.connect(lambda: self.command(title))
    ...
    def command(self, title:str):
        ...
        if self.playbutton.play_status == Status.START:
            ...
            self.signal.signal.emit(
                f"/home/filip/Documents/qt-learning/songs/{title}.mp4"
            )
            # self.movieopener(f"/home/filip/Documents/qt-learning/songs/{title}.mp4")
...
class PanelWithSongs:
    def __init__(self, master, main_layout):
        ...
        print(self.scroll.verticalScrollBar())
        ...
        layout = ForLabel(self.tab, "ID", "TITLE", "AUTHOR", "TIME")
