...
class VideoPlayer(QWidget):
    ...
    def wheelEvent(self, event):
        ...
        if self.widescreen == True:
            self.setGeometry(
                mleft, mtop, mwidth + mscale, round((mwidth + mscale) / 1.778)
            )
        else:
            self.setGeometry(
                mleft, mtop, mwidth + mscale, round((mwidth + mscale) / 1.33)
            )
    ...
    def printMediaData(self):
        if self.mediaPlayer.mediaStatus() == 6:
            if self.mediaPlayer.isMetaDataAvailable():
                res = (
                    str(self.mediaPlayer.metaData("Resolution"))
                    .partition("PyQt5.QtCore.QSize(")[2]
                    .replace(", ", "x")
                    .replace(")", "")
                )
